...
class EDaicWozDataset(AcumenDataset):
    def __init__(self, args, kind = 'train', data_transforms = None):
        ...
        if kind == 'train':
            self.df = pd.read_csv(f'{args.environment["e-daic-woz"]}/splits/train_split.csv')

        if kind == 'validation':
            self.df = pd.read_csv(f'{args.environment["e-daic-woz"]}/splits/val_split.csv')
            self.df = self.df.sort_values(by = 'duration')

        if kind == 'test':
            self.df = pd.read_csv(f'{args.environment["e-daic-woz"]}/splits/test_split.csv')
            self.df = self.df.sort_values(by = 'duration')
    ...
    def _priority_modalities(self):
        ...
        if len(self.args.modalities) == 1:
            ...
            raise Exception("No modality was identified when computing the presence mask")
